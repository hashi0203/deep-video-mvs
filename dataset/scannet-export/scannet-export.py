...
class SensorData:
    ...
    def export_train(self, output_path, frame_skip):
        ...
        for index in range(0, len(self.frames), frame_skip):
            ...
            depth = np.frombuffer(depth, dtype=np.uint16).reshape(self.depth_height, self.depth_width)
...
if is_train:
    input_path = "/home/share/dataset/Matterport3D/v1/scans"
    output_path = "/home/nhsmt1123/master-thesis/deep-video-mvs/data/scannet"
else:
...
if __name__ == '__main__':
    ...
    scene_names = sorted(os.listdir(input_path))
    ...
    if is_train:
        ...
        for scene_name in scene_names:
            scene_path = os.path.join(os.path.join(input_path, scene_name), "sens")
            if not(os.path.exists(scene_path)):
                print("extracting %s/sens.zip" % scene_name)
                with zipfile.ZipFile(scene_path + ".zip") as existing_zip:
                    existing_zip.extractall(scene_path[:-4])
            for sequence_name in sorted(os.listdir(scene_path)):
                sequence_paths.append(os.path.join(scene_path, sequence_name))
                idx = sequence_name.split('_')[-1][:-5]
                if scene_name in scene_names_dict:
                    scene_names_dict[scene_name].append(idx)
                else:
                    scene_names_dict[scene_name] = [idx]
